...
def get_modulation_and_datarate(interface, ieee_standard, ht_cap):
    ...
    return modulation if modulation else 'N/A', datarate if datarate else 'N/A'
...
def topology(num_cars, sumo_config_file, flood_type, duration, rate_mbps, unicast=True,
    ...
    info("*** Creating Links\n")
    for car in net.cars:
        #print(f"Configuring mesh with freq={5180}, channel={36}, ht_capab={car.wintfs[0].ht_capab}")
        print(f"Configuring mesh with freq={5180}, channel={36}")
        ...
        net.addLink(car, intf=car.wintfs[0].name, mode='ac', cls=mesh, ssid='mesh-ssid', channel=36, freq=5180, ht_cap='HT40+')
        #net.addLink(car, intf=car.wintfs[0].name, mode='ac', cls=mesh, ssid='mesh-ssid', channel=36, freq=5180, ht_capab=car.wintfs[0].ht_capab)
